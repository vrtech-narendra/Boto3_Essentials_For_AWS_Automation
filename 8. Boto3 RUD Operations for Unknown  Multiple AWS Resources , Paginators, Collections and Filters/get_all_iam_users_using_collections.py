...
try:
    ...
    # user_iterator = iamResource.users.all()
    # user_iterator = iamResource.users.filter(PathPrefix='/devops/')
    # user_iterator = iamResource.users.limit(10)
    user_iterator = iamResource.users.page_size(1000)
    cnt=1
    for eachUser in user_iterator:
        print(cnt,eachUser.user_name)
        cnt+=1
    if cnt == 1:
        print(f"There are no iam users")
