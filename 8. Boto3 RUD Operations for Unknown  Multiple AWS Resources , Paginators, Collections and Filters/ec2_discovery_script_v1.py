    ...
    for eachPage in response_iterator:
        for eachReservation in eachPage.get('Reservations'):
            for eachInstance in eachReservation.get('Instances'):
                instance_id = eachInstance.get('InstanceId', '')
                state = eachInstance.get('State', {}).get('Name', '')
                instance_type = eachInstance.get('InstanceType', '')
                architecture = eachInstance.get('Architecture', '')
                image_id = eachInstance.get('ImageId', '')

                instances.append([ cnt, instance_id, state, instance_type, architecture, image_id ])
                cnt += 1
    # Write to CSV
    if cnt == 1:
        print(f'There are no  EC2 Instnaces under region {region} for given aws account/profile {profileName}')
        sys.exit(0)
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['S.No', 'Instance_ID', 'State', 'Type', 'Architecture', 'AMI_ID'])
        writer.writerows(instances)

    print(f"EC2 Inventory written to: {output_file}")
