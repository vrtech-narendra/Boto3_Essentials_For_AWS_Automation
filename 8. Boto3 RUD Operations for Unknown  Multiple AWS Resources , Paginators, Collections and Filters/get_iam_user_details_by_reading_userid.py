...
def main():
    try:
        ...
        parser.add_argument('-u', '--userId', required=True, help="IAM User Name to get Details")
        ...
        for eachPage in responses:
            for eachIamUser in eachPage.get('Users'):
                eachIAMUserId = eachIamUser.get('UserId')
                if eachIAMUserId == userId:
                    print(f"UserName    : {eachIamUser.get('UserName')}")
                    print(f"UserId      : {eachIamUser.get('UserId')}")
                    print(f"UserArn     : {eachIamUser.get('Arn')}")
                    print(f"CreatedAt   : {eachIamUser.get('CreateDate')}") 
                    userFound=True 
                    break    
            if userFound:
                break     
        if not userFound:
            print(f"Sorry!!!, We are unable to find user details for given IAM UserId. Please check your UserId once")
