...
def main():
    try:
        ...
        #list_objects is old method
        #list_objects_v2 is new method - prefere this
        paginator = s3Client.get_paginator('list_objects_v2')
        response_iterator = paginator.paginate( Bucket=bucketName,Prefix='configs/')
        cnt=1
        for eachPage in response_iterator:
            for eachBucketObject in eachPage.get('Contents'):
                if eachBucketObject.get('Key') == 'configs/':
                    continue
                print(cnt,eachBucketObject.get('Key'))
                cnt+=1
        if cnt == 1:
        ...
        if userChoice == "download":
            ...
            s3Client.download_file(bucketName, objectName, 'downloadfile.txt')
        elif userChoice == "delete":
            objectName = input("Enter your object to delete: ")
            print(f"Deleting object {object} from bucket {bucketName}...")
            response=s3Client.delete_object(Bucket=bucketName, Key=objectName)
            # print(response)
